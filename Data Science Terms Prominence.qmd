...
Moreover, agenda-setting theory has applications in various fields, including politics, journalism, and public relations. It has been used to study the influence of the media on political campaigns, the framing of news stories, and the communication of consent [@Caldwell1996; @Meraz2011; @McCombs1997]. The theory has also been examined in online news media, where researchers have analyzed agenda-setting effects among different platforms [@Lim2006].

In recent years, there has been a growing interest in meta-analyses of agenda-setting research, which provide a comprehensive overview of the existing empirical literature. These meta-analyses analyze the evolution of agenda-setting theory in terms of research focus, subject areas, and research methods. By synthesizing and analyzing statistical indicators from many studies, these meta-analyses contribute to a deeper understanding of the current state of agenda-setting research [@Li2022].

# Methodology

## Selection of newspapers

The selection of newspapers for a study focused on data science-related terms is crucial for comprehensive coverage and depth of analysis. This research has chosen the Philippine Daily Inquirer website for several reasons. Firstly, the Philippine Daily Inquirer is one of the most widely read newspapers in the Philippines, ensuring the analysis is relevant to a broad online audience. Additionally, the extensive archives of the newspaper's online version are essential for historical data analysis, allowing for an exploration of the evolution of data science terminology over time. The diverse array of articles published on the website, including news, opinion pieces, features, and editorials, is vital for understanding the different contexts in which data science-related terms are used. As a continually updated digital news source, the Philippine Daily Inquirer website offers the most current perspective on using and understanding data science terms. Moreover, the newspaper's national scope reflects national interests and discourse, providing a comprehensive overview of how data science is perceived and discussed nationwide. The editorial stance and policies of the newspaper provide insight into how data science topics are framed and prioritized, which is an essential aspect of media analysis.

## Time frame of the study

The time frame for the study, spanning from 1993 to 2023, is crucial for capturing the evolution of data science in the Philippine media landscape. This extended period allows for identifying long-term trends and patterns in media coverage related to data science [@Ho2007]. Additionally, the chosen period should align with significant events or developments in the field of data science, such as the introduction of data science courses, government initiatives, or the establishment of data science firms in the Philippines.

The start and end dates of the data collection period should be justified based on the emergence of critical events or milestones in the Philippine data science landscape [@Suppli2018]. Furthermore, dividing the time frame into segments, such as monthly or quarterly intervals, allows for a detailed temporal analysis, enabling the examination of changes in reporting frequency or contextual shifts around specific events.

The selected time frame may influence the results and the interpretation of trends in the data, as it allows for the assessment of any immediate effects of significant global or local events on the presence and representation of data science in the press. Considerations for seasonal variations or particular circumstances, such as elections or natural disasters, should also be factored in, as they might affect media coverage and public interest in data science.
...
This process begins with the careful selection of primary and secondary keywords, such as "**Data Science**", "**Big Data**", "**Analytics**", "**Artificial Intelligence**", along with related terms like "**Robotics**". The search for these keywords is facilitated by using the search textbook of <https://www.inquirer.net/>. A manual review process is implemented to validate the accuracy and relevance of the automated search results. This involves random checks to ensure articles are contextually related to data science and its associated fields. Moreover, the entire process is conducted with strict adherence to ethical and legal considerations, ensuring compliance with copyright laws and respect for privacy and ethical guidelines, especially when handling articles that include interviews or personal data. Through these comprehensive keyword search techniques, the study aims to gather a wide range of data from the Philippine Daily Inquirer, offering detailed insights into the media's portrayal and emphasis on data science and related terms.
...
The `rvest` package [@rvest2022] in `R` was utilized for web scraping and HTML data extraction. This package simplifies the process of reading and manipulating HTML content, making it invaluable for gathering data from web sources. Version 1.0.3 of rvest was explicitly used.
...
The primary operating system used for all data processing tasks was `Windows 11 x64 (build 22621)`. This version of Windows offers enhanced security features, improved performance, and a user-friendly interface, which significantly contributed to the efficiency of our data handling processes. The 64-bit architecture was particularly beneficial for handling large datasets, ensuring memory-intensive operations were executed smoothly.

All scripts and processed data files have been made publicly available for transparency and to facilitate the reproducibility of our findings. They are hosted on GitHub, a widely recognized version control and code-sharing platform. The repository can be accessed at <https://github.com/dddorado/ds_term_prominence>. This repository includes all the scripts used in the data processing and analysis phases, along with detailed documentation on the usage and functionality of these scripts [@Dorado2023].

## Data Scrapping Process

In this study, we aimed to systematically gather data from <https://www.inquirer.net/>, focusing on identifying and downloading web pages that contain specific keywords of interest. This task was necessary due to the nature of the website's content presentation. Like many contemporary news and information websites, Inquirer.net leverages modern web technologies to enhance user experience and content delivery. A vital aspect of these technologies is the use of `JavaScript` to load and display content dynamically. This approach, commonly employed in web applications developed with frameworks like `Angular`, `React`, or `Vue`, poses a specific challenge for web scraping tools like `rvest`. Once the pages are downloaded, we can proceed to harvest its content.
...
The code starts by creating a vector `robotics_url` containing a URL list. Each URL points to an HTML page that presumably includes robotics-related articles.
...
An `extractor` function is defined to process each URL. This function uses `read_html` from the `rvest` package to parse the URL's HTML content. It then uses a series of `rvest` functions to extract specific data:
...
- The `unique()` function is applied to the titles to ensure no duplicates.
...
The `lapply` function applies the `extractor` function to each URL in the `robotics_url` vector. This results in a list of tibbles, where each tibble contains data scraped from one URL.
...
Finally, the code adds a new column named `category` to the combined tibble, assigning the value 'robotics' to each row. This categorization step indicates that all scraped data in this dataset are related to robotics.
...
We started with a script that sets up a list of additional stopwords specific to the context (like "ai", "inquirer", etc.) and combines them with the default English stopwords provided by the `tm` package.
...
We then tokenizes the cleaned titles and calculate term frequencies. It uses `unnest_tokens` to break down the titles into words and then counts the frequency of each word within each category.
...
We select the top 10 terms for each category based on their frequency using `top_n`.
...
Finally, we use `ggplot2` for data visualization. It creates a bar plot showing the top 10 terms for each category, with words on the x-axis, frequency on the y-axis, and different colors for each type. The plot uses `facet_wrap` to create separate plots for each category and `coord_flip` for better readability of the terms.

```{r Data Visualization, eval=TRUE, echo=TRUE, fig.height=5, fig.width=6}
...
We then perform a temporal analysis of the occurrence of news articles related to the keywords across the years.

```{r Year Extraction, eval=TRUE, echo=TRUE, results='hide'}
compiled_dataset$year <- year(compiled_dataset$date)
```

The code starts by manipulating the `compiled_dataset`, specifically by adding a new column named `year`. This column is created by extracting the year from the `date` column in the dataset. The `year` function from the `lubridate` package, is used for this extraction. This step is crucial for temporal analysis, as it allows the dataset to be analyzed on a yearly basis.

```{r Data Grouping and Summarization, eval=TRUE, echo=TRUE, results='hide'}
yearly_data <- compiled_dataset %>%
    group_by(year, category) %>%
    summarise(count = n())
```

The first part of the code uses `dplyr` for data processing. The `group_by(year, category)` function groups the `compiled_dataset` by two columns: `year` and `category`. This means the dataset is partitioned into subsets based on the combination of year and category. The `summarise(count = n())` function then calculates the count `(n())` of rows (articles) in each group, effectively giving the number of articles per category for each year. The result of this operation is stored in `yearly_data`.

```{r Data Visualization with ggplot2,eval=TRUE, echo=TRUE}
ggplot(yearly_data, aes(x = year, y = count, group = category, color = category)) +
    geom_line() +
    labs(title = "Yearly Temporal Analysis by Category", x = "Year", y = "Count") +
    theme_minimal()
```

The second part of the code creates a bar chart using `ggplot2`. The `aes(x = year, y = count, fill = category)` sets up the aesthetics for the plot, specifying that the x-axis will represent years, the y-axis will represent counts, and different colors (fill) will represent different categories. The `geom_bar(stat = "identity", position = "dodge")` adds bar geometries to the plot, with bars positioned side-by-side (dodged) for each year-category combination rather than stacked. This makes it easier to compare the count of articles across categories for each year. `theme_minimal()` is used to apply a minimalistic theme to the plot, a stylistic choice for a clean and uncluttered presentation. The `labs()` function adds labels to the plot, setting the title of the plot as "Yearly Article Count by Category" and naming the x and y-axes as "Year" and "Count," respectively.

The prominent occurrence of terms related to data science and similar fields in Philippine newspapers suggests several vital implications. Firstly, it indicates a growing interest and focus on technology and data science within the country, reflecting the global trend towards data-driven decision-making and artificial intelligence. This prominence could indicate heightened public awareness and an educational shift towards these disciplines. Additionally, it might signal the alignment of the Philippines' economic and industrial trends with global technological advancements.

Furthermore, the frequent coverage of these topics could reflect governmental focus and investment in the tech industry, hinting at potential public policy shifts and infrastructure development to support technological growth. The media's role in shaping public perception is also crucial, as this increased focus could influence more individuals to pursue careers in data science and related fields. This trend might also suggest a vibrant business and start-up environment in the tech sector of the Philippines, highlighting growth areas for investment and job creation.

Moreover, the frequent discussions around data science and associated fields might point to societal challenges and opportunities, such as data privacy, ethical AI use, and the need for skilled professionals. Finally, this trend could signify the Philippines' increasing integration into the global tech landscape, indicating its responsiveness to international technological advancements. While the term frequency analysis underscores the prevalence of these topics, it is essential to conduct further qualitative research to understand the context and sentiment behind this media coverage fully.

# Recommendations

In light of the findings, several recommendations can be proposed. Firstly, educational institutions and policymakers are advised to emphasize emerging technologies like data science, AI, and robotics. This could entail updating educational curricula to include advanced technology subjects and promoting STEM education more vigorously. Additionally, there is an opportunity for media outlets and educational bodies to collaborate on public awareness initiatives to enhance understanding and appreciation of these fields among the general public.

Investments in local research and development within these technological domains should be a priority for both the government and the private sector, aiming to keep pace with global trends and establish competitive advantages in specific areas of these technologies. Furthermore, the nurturing and support of technology startups, through funding and mentorship, can stimulate innovation and economic growth in these sectors.

An equally important recommendation is the need to focus on the ethical implications of data science and AI advancements. This could involve forming regulatory bodies to ensure these technologies' responsible development and implementation. Strengthening industry-academia collaboration would also be beneficial, as it would lead to more practical research outcomes, internship opportunities, and enhanced job prospects for students in these fields. 

Finally, the Philippines could greatly benefit from international collaborations and partnerships in research and policy-making in data science and technology, bringing new perspectives, technologies, and investments to the country. These recommendations are geared towards leveraging the growing media interest in data science and related technologies, to foster technological advancement, education, and ethical practices.
